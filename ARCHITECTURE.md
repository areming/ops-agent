...
┌────────────────────────── 你的本地机器 ──────────────────────────┐
│                                                                  │
│   opsagent connect <host>                                        │
│   ┌──────────────────────────────┐                              │
│   │  CLI 客户端 (薄)              │   bubbletea TUI / REPL       │
│   │  - 渲染流式对话              │   收发 Frame 协议            │
│   │  - 高危操作确认弹窗          │                              │
│   └──────────────┬───────────────┘                              │
└──────────────────┼──────────────────────────────────────────────┘
                   │  SSH (你已有的密钥认证)
                   │  在远端运行 `opsagent _bridge`，
...
| **Config** | 配置加载（providers、巡检、白名单、运行身份等） | TOML |

---

## 3. 数据模型

### 3.1 SQLite（默认 `~opsagent/.opsagent/state.db`）
...
| 命令 | 作用 |
|---|---|
| `opsagent enroll <host>` | 一键部署：传二进制、建专用用户、写 sudoers 白名单、装 systemd、初始化配置 |
| `opsagent connect <host>` | 连上某台 agent 对话（日常主入口） |
| `opsagent serve` | 在服务器上以常驻 agent 运行（由 systemd 拉起） |
| `opsagent _bridge` | 内部：远端把 SSH stdio 桥到 unix socket（不直接用） |
| `opsagent key set/list` | 管理加密的模型 API key |
| `opsagent todos` / `logs` | 查自愈待办 / 审计日志 |
...
| **key 安全** | API key 加密存 keystore（NaCl secretbox / age），重启自恢复。 |
...
```
ops-agent/
├── cmd/opsagent/main.go        # 入口，子命令分发
├── internal/
│   ├── cli/                    # 薄客户端 + tui/ (bubbletea)
...
2. **配置格式**：我默认 **TOML**（Go 生态友好、注释清晰）。可改 YAML。
