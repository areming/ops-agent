...
### 3.2.1 自定义命令（Markdown，`/<名称>` 触发）
- 路径：`StateDir/commands/*.md`（`OPSAGENT_COMMANDS_DIR` 可覆盖）；与模型/记忆一样**存在 agent 侧**，本地与 `connect <host>` 远端会话都生效。
- 格式：极简 frontmatter（`name` / `description`）+ 正文；正文是自然语言、shell 或混合。
- 触发：客户端把非内置的 `/<名称>` 作为 `RunCommand` 帧发给 agent；agent **每次重读目录**（随放随用，无需重启）查找命令，把其正文注入成本轮 user 输入，跑**正常 turn**——所以工具、安全闸门、确认、审计**一律照旧适用，命令不绕过任何安全机制**。未知名字回 Error+Done，客户端干净返回提示。
- 列举：`/commands`（及 `/help` 末尾）经 `command.list` 控制帧取列表，纯展示。
...
// 传输帧
type Frame struct {
    // UserInput|AssistantDelta|ToolStart|ToolOutput|ConfirmRequest|ConfirmReply|
    // ControlRequest|ControlReply|RunCommand|Cancel|Done|Error
    // Cancel（cli→agent，ESC/Ctrl-C）中断运行中的一轮：取消该轮 ctx，停模型流、
    // kill 正在跑的命令，仍以 Done 收尾。
    // RunCommand（cli→agent）触发自定义命令：与 UserInput 一样开一整轮（不是
    // ControlRequest 的单条回复），agent 注入命令定义后跑正常 turn。
    Type    FrameType
...
│   ├── memory/                 # store(sqlite) / knowledge / commands / audit
│   ├── secret/                 # keystore
│   └── config/                 # 配置加载
├── go.mod
└── README.md
