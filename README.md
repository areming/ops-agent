...
交叉编译到 `./dist/ops-{linux-amd64,linux-arm64,windows-amd64}`，每个都是单静态二进制（`file dist/ops-linux-amd64` 显示 `statically linked`）。

Windows 上可选一键本地安装（把 `ops` 放进 PATH + 启用 ssh-agent，之后直接敲 `ops`）：
