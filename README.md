...
对话内 `/model` 打开模型面板，末行「+ 新增模型…」可**直接配一个新厂商/模型的档案**（选 provider → 模型 → 填 base URL/key），添加完即切过去；`d` 删除不要的档案，`↑/↓`+Enter 在已存档案间切换。新增的 key 按档案分别加密存进 keystore，无需手改文件。这对本地会话和 `ops connect <host>` 连上的远端 daemon 都生效（远端 key 经 SSH 通道传过去由 daemon 入库）。

> **注（enroll 过的机器）**：当前 enroll 仍把 `OPSAGENT_PROVIDER`（以及给了 `--model` 时的 `OPSAGENT_MODEL`）写进 systemd unit 的 `Environment=`，按优先级**盖过 `config.json`**。所以在 enrolled 远端用面板切换/新增，当次会话生效，但**重启后活跃模型会被 unit 的 env 还原**。要让远端的档案选择持久，目前仍需重跑 enroll（后续版本会改为 enroll 把档案种进 daemon 的 config.json，不再钉 env）：
>
> ```bash
