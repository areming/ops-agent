...
对话里支持 `/命令`（对齐常见 CLI）：`/model` 打开模型面板（↑/↓ 选择、Enter 切换、d 删除、末行新增；带名称 `/model <名称>` 直接切换）、`/commands` 列出自定义命令、`/logs [N]` 看操作日志、`/clear` 清空当前对话、`/help`、`/quit`。作用于当前会话所连的那台机器（本地或远端）。

**自定义命令**：在 agent 的 commands 目录（`StateDir/commands/`，默认随机器；`OPSAGENT_COMMANDS_DIR` 可覆盖）放 `*.md` 文件，即可用 `/<文件名或 name>` 触发。文件用极简 frontmatter + 正文：

```markdown
---
name: diskcheck
description: 检查磁盘占用并报告最满的挂载点
---
查看各挂载点磁盘使用率，找出最满的，简要报告。只读，别改动任何东西。
```

正文可以是自然语言指令、shell 脚本，或两者混合——触发时它作为本轮任务注入，模型据此理解“要做什么、操作范围在哪”，涉及系统变更的步骤照常过安全闸门与确认（自定义命令不绕过任何安全机制）。`/<名称> 附加文字` 会把附加文字一并带给模型（如 `/deploy staging`）。文件随放随用，无需重启 daemon。`/commands` 看当前可用列表。

巡检与 fan-out 的细节（边界、默认安全、验证脚本）见 [ONBOARDING.md](ONBOARDING.md) §6/§7。

## 配置

配置优先级 **环境变量 > `config.json`（StateDir 下）> 内置默认**。模型以**档案**形式存进 `config.json`（每个档案 = provider/model/base_url + 一个 keystore 条目引用 + 活跃指针），引导/`/model` 面板的新增、切换、删除都落到这里；API key **不**进配置，按档案分别加密存在 keystore（旧的单一 `api_key` 条目会自动迁移成一个 `default` 档案）。诊断模型三件仍单独存。常用环境变量：
...
| `OPSAGENT_COMMANDS_DIR` | `StateDir/commands` | 自定义命令（`*.md`）目录，`/<名称>` 触发 |

完整列表（含 state/db/knowledge 路径等）见 [ONBOARDING.md](ONBOARDING.md) §5。

### 换 key / 换厂商 / 加模型

模型以**档案**形式管理，每个档案的 key 单独加密存在 keystore（条目名按档案派生，不再是固定的 `api_key`）。日常增删改全在 `/model` 面板里完成，无需手改文件。

**① 换 key（厂商/模型不变）或换厂商 / 加模型——都用 `/model` 面板**

`/model` 打开面板：`↑/↓`+Enter 在已存档案间切换、`d` 删除、末行「+ 新增模型…」配一个新档案（选 provider → 模型 → 填 base URL/key）。**换 key 就是「新增」一个同 provider/同模型的档案填上新 key**——它会**就地更新**那个档案的 key（不会重复），并切过去。新增/更新对本地会话和 `ops connect <host>` 连上的**远端 daemon 都生效且持久**（远端 key 经 SSH 通道交给 daemon 入库，写进它的 `config.json`/keystore，重启保留）。

**② 远程也可重跑 enroll（幂等）**

把首个档案重新种到远端、或脚本化换 key：

```bash
echo "$ANTHROPIC_KEY" | ops enroll <host> --provider anthropic --model claude-... [--base-url ...]
```

enroll 把模型档案种进 daemon 的 `config.json`（**不再**在 systemd unit 里钉 `OPSAGENT_PROVIDER`/`OPSAGENT_MODEL`），相同 provider/模型会就地更新 key、不同则新增并切换。

> **注（用旧版本 enroll 的老机）**：v0.0.15 及更早 enroll 的 unit 仍钉着 `OPSAGENT_*`，会**盖过 `config.json`**，面板切换在那种机器上重启会被还原——用新版 `ops`（v0.0.16+）**重跑一次 enroll** 即可迁移（去掉 env 钉、改为种 config.json）。`OPSAGENT_*` 环境变量保留为临时覆盖逃生口。
