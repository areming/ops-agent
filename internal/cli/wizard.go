...
func collectModel(r *bufio.Reader) (providerInfo, string, string, error) {
	entry, ok := selectProvider(r)
	if !ok {
		return providerInfo{}, "", "", errWizardCanceled
	}
	model, err := selectModel(r, entry)
	if err != nil {
...
// selectProvider shows the catalog as an arrow-key menu (each row labeled with
// its default base URL), falling back to a numbered prompt off a terminal.
func selectProvider(r *bufio.Reader) (providerInfo, bool) {
	opts := make([]menuOption, len(providerCatalog))
	for i, e := range providerCatalog {
		desc := e.BaseURL
		if desc == "" {
			desc = "手动填写 OpenAI 兼容地址"
		}
		opts[i] = menuOption{label: e.Label, desc: desc}
	}
	idx, ok := selectMenu(r, "选择模型提供商", opts, 0)
	if !ok {
		return providerInfo{}, false
	}
	return providerCatalog[idx], true
}
...
func selectModel(r *bufio.Reader, entry providerInfo) (string, error) {
	if len(entry.Models) == 0 {
		return prompt(r, "模型名", entry.DefaultModel)
	}
	...
	idx, ok := selectMenu(r, "选择模型", opts, initial)
	if !ok {
		return "", errWizardCanceled
	}
	if idx == len(entry.Models) { // the "Custom" row
		return prompt(r, "模型名（手动输入）", entry.DefaultModel)
	}
...
// descriptions) so onboarding matches the rest of the UI. ok is false on
// Esc/Ctrl-C. Off a terminal (or if raw mode can't be entered) it falls back to
// a numbered prompt read from r.
func selectMenu(r *bufio.Reader, title string, opts []menuOption, initial int) (int, bool) {
	fd := int(os.Stdin.Fd())
	...
	fmt.Printf("\r\n%s %s  %s\r\n", accent("◆"), bold(title), muted("↑/↓ 选择 · Enter 确认 · Esc 取消"))
	...
	for {
		...
		switch {
		case buf[0] == 3, buf[0] == 27 && n == 1: // Ctrl-C or lone Esc → cancel
			return 0, false
		case buf[0] == '\r', buf[0] == '\n':
			return sel, true
		...
		fmt.Printf("\x1b[%dA", len(opts)) // back to the first option line
		render()
	}
}
...
// (initial); the loop re-asks on an out-of-range entry.
func numberedSelect(r *bufio.Reader, title string, opts []menuOption, initial int) (int, bool) {
	fmt.Printf("\n%s\n", title)
	for i, o := range opts {
