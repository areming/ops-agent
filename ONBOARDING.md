...
ops uninstall [--purge]                     卸载；--purge 连同全部数据一并删除（执行前需手输 yes 确认）
```

## 5. 配置（环境变量 + config.json）

配置优先级 **环境变量 > `config.json`（StateDir 下）> 内置默认**。模型以**档案**形式存进 `config.json`（`{active, models[]}`，每个档案 provider/model/base_url + keystore key 引用），引导与会话内 `/model` 面板的新增/切换/删除都落到这里。enroll 现在把模型档案**种进 daemon 的 `config.json`**、**不再**在 systemd unit 里钉 `OPSAGENT_PROVIDER/MODEL/BASE_URL`，所以 enrolled 远端的面板切换/新增重启后也保持（unit 只留 `OPSAGENT_STATE_DIR` 和可选的 `OPSAGENT_PATROL_SERVICES`/`OPSAGENT_DIAG_MODEL`；`OPSAGENT_*` 仍是临时覆盖逃生口。v0.0.15 及更早 enroll 的老机需重跑一次 enroll 迁移）。引导（本地 `ops` / `ops setup`）会**上下键**列出主流 provider（DeepSeek / OpenAI / Anthropic / Moonshot / Qwen / z.ai / Gemini / Grok 等）并预填各家 base URL，模型也是列表选择 + Custom 手填；第三方 OpenAI 兼容商底层统一记 `provider=openai` + 对应 base URL，所以下表的 provider 取值仍是这三个 adapter：
