...
配置优先级 **环境变量 > `config.json`（StateDir 下）> 内置默认**。模型以**档案**形式存进 `config.json`（`{active, models[]}`，每个档案 provider/model/base_url + keystore key 引用），引导与会话内 `/model` 面板的新增/切换/删除都落到这里，systemd 注入的 `OPSAGENT_*` 仍优先（enrolled 远端因此重启会还原活跃档案，见 README §换厂商）。引导（本地 `ops` / `ops setup`）会**上下键**列出主流 provider（DeepSeek / OpenAI / Anthropic / Moonshot / Qwen / z.ai / Gemini / Grok 等）并预填各家 base URL，模型也是列表选择 + Custom 手填；第三方 OpenAI 兼容商底层统一记 `provider=openai` + 对应 base URL，所以下表的 provider 取值仍是这三个 adapter：
